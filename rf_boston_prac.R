...
library(caret)
nearZeroVar(boston_test_norm, saveMetrics = TRUE) # all_FALSE (제거대상 없음) 분산이 0에 가까운 변수 삭제
...
sample <- as.data.frame(sample) #sample을 data프레임으로 바꿔준다.

head(sample)
cor(boston_test$price, sample$medv) #0.8424729
